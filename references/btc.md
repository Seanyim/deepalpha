...
1. **Fetch each indicator** via web_search from the sources listed below. Pull the latest raw value for all 13 indicators.
2. Normalize each raw value to a 0-100 sub-score using the ranges defined above, then apply weights.
3. Sum to composite: `totalScore` (0-100), split into `dailyScore` (0-32, fast pulse) and `weeklyScore` (0-68, slow structure).
...
- Pull each indicator via web_search; prefer free public sources listed above
