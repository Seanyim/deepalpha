...
## [deepalpha extension] Fundamental depth — primary-source (SEC XBRL via `data-layer.md`)

Pull statements from SEC XBRL first (edgartools/secedgar/stockscope), not scraped pages. Add these to the four core VALUE dimensions:

### Piotroski F-Score (0–9 — financial-strength gate)
+1 each: positive net income · positive operating CF · ROA up YoY · CFO > net income (earnings quality) · lower long-term debt ratio · higher current ratio · no new share dilution · higher gross margin · higher asset turnover. **≥7 strong, ≤3 weak.** Fast quality filter before deeper work.

### ROIC & reinvestment
ROIC = NOPAT ÷ invested capital. Want ROIC > WACC and ideally >15% sustained 3yr, with a reinvestment runway. ROIC beats ROE as a quality read when leverage is flattering ROE.

### Accounting quality / red flags (10-K/10-Q text + cash-flow)
FCF/NI <100% over time (accrual risk) · receivables/inventory growing faster than revenue · rising "other"/one-time items · restatements · widening GAAP-vs-non-GAAP gap · aggressive revenue recognition · auditor change / material-weakness language (search filing text). Pull MD&A + risk factors via the data layer.

### Dividend safety (income names)
Payout (<60% earnings, <70% FCF healthy) · FCF coverage · debt load · dividend-growth streak & CAGR · yield-trap check (high yield + deteriorating fundamentals = warning).

### Moat (Porter's Five Forces lens)
Score brand, network effects, cost advantage, switching costs, intangibles/regulatory. **2+ durable sources = wide moat.** Cross-check pricing power (gross-margin stability) and market-share trend. Five Forces: rivalry, entrants, substitutes, supplier power, buyer power.

### Short interest (contra-signal)
Short % of float, days-to-cover. High+rising = bear conviction or squeeze fuel; pair with the thesis — don't be the last long in a crowded short, don't chase a squeeze without a fundamental anchor.
