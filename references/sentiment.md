...
## [deepalpha extension] Breadth, market-timing & social sentiment

The five survey indicators above gauge *crowd positioning*. Add tape-based breadth and social reads (mechanics in `technical.md` §B–C):

- **Breadth health** — % above MA50/MA200, A/D line, new-high/new-low, sector participation. Narrowing breadth in an uptrend = distribution warning.
- **Market-top (O'Neil distribution days)** — 5–6 higher-volume index down-days in ~25 sessions + leading-stock breakdown = probable top.
- **Market-bottom (Follow-Through Day)** — index +1.5%+ on rising volume, day 4–7 of a rally attempt = re-entry greenlight.
- **Bubble/euphoria (Minsky–Kindleberger)** — quant 0–12 (put/call, VIX, margin debt, breadth divergence, IPO/SPAC heat, valuation) + small qualitative add; phases Normal→Critical with risk budgets.
- **Social sentiment (lead, not proof)** — Reddit / X / news / Polymarket tone as an early signal; confirm against breadth & fundamentals (evidence ladder in `theme.md`). Extreme retail euphoria = caution; capitulation = opportunity scan.

Combine into the SENTIMENT verdict as "X/N warnings" plus a market-timing posture (healthy / late-cycle / topping / bottoming).
